...
try {
    //1. 测试创建区块
    console.log("1. 测试创建区块: ");
    const genesis = Block.genesis;
    console.log("--索引: ", genesis.index);
    console.log("--数据: ", genesis.data);
    console.log("--暗带: ", genesis.hash);
    console.log("--前一区块暗带: ", genesis.previousHash);
    console.log("--nonce: ", genesis.nonce);
    console.log("测试创建区块创建成功: ++++++++++++++++++++")
    ...
    //3. 测试暗带难题验证函数
    console.log("3. 测试暗带难题验证函数: ");
    console.log("--当前需要更新: ", blockchain.difficulty, "个前号0");
    const validHash = "000abcl23def";
    const invalidHash = "00abcl23def";
    // hash值前面码。大于等于3的时候，会返回true。小于3的时候，会返回false
    ...
    console.log("测试暗带难题验证函数成功: ++++++++++++++++++++");
    
    //4. 测试暗带计算函数
    console.log("4. 测试暗带计算函数: ");
    const testHash = blockchain.calculateHash(0, '0', 1234567890, "测试数据", 0);
    console.log("--计算出的暗带值: ", testHash);
    console.log("--暗带长度: ", testHash.length, "字符");
    console.log("测试暗带计算函数成功: ++++++++++++++++++++");
    
    //5. 测试第一个区块
    console.log("5. 测试第一个区块: ");
    console.log("---开始挖矿---");
    console.time("+挖矿耗时+*");
    blockchain.mine("第一个测试区块:");
    console.timeEnd("+挖矿耗时+*");
    console.log("---新区块数据: ", blockchain.latestBlock.index);
    console.log("---新区块数据: ", blockchain.latestBlock.data);
    console.log("---新区块暗带: ", blockchain.latestBlock.hash);
    console.log("---新区块nonce: ", blockchain.latestBlock.nonce);
    console.log("---当前区块链的长度: ", blockchain.get().length);
    console.log("测试第一个区块成功: ++++++++++++++++++++");
    
    //6. 测试第二个区块
    console.log("6. 测试第二个区块: ");
    console.log("---开始挖矿---");
    console.time("+挖矿耗时+*");
    ...
    console.log("---第二个区块暗带: ", blockchain.latestBlock.hash);
    console.log("---第二个区块nonce: ", blockchain.latestBlock.nonce);
    console.log("---当前区块链的长度: ", blockchain.get().length);
    console.log("测试第二个区块成功: ++++++++++++++++++++");
